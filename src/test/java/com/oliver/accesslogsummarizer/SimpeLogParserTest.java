...
public class SimpeLogParserTest {
	...
	public void testParser() {
		...
		map = parser.parseLog(Arrays.stream(log), new ParsingOptions(6, 9));
	...
	public void testIncorrectParseOptions() {
		AccessLogParser parser =  new SimpleLogParser();
		map = parser.parseLog(Arrays.stream(log), new ParsingOptions(6, 7));
	...
	public void testIndexOutOfRange() {
		AccessLogParser parser =  new SimpleLogParser();
		map = parser.parseLog(Arrays.stream(log), new ParsingOptions(6, 10));
		assertEquals(0, map.size());
	}
